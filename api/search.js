...
function parseHadithInfo($, infoEl) {
  ...
  $(infoEl).contents().each((_, node) => {
    if (node.type === "tag" && $(node).hasClass("info-subtitle")) {
      ...
      const val = node.type === "text"
        ? node.data.trim()
        : node.type === "tag" ? $(node).text().trim() : "";
      if (val) { fields[currentLabel] = val; currentLabel = null; }
    }
  });
  return fields;
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    $("div.hadith-info").each((_, infoEl) => {
      ...
      const arabic_text = htmlToText($(infoEl).prev("div.hadith").html() || "")
        .replace(/^\d+\s*[-–]\s*/, "").trim();
