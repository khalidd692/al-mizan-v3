...
// ── sseChunk — relay d'un chunk texte brut LLM (animation barre de progression)
function sseChunk(res, idx, chunk) {
  const payload = "event: chunk\ndata: " + JSON.stringify({ index: idx, text: chunk }) + "\n\n";
  res.write(payload);
  if (typeof res.flush === "function") res.flush();
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// normalizeMatn — strip tashkil pour déduplication
// ═══════════════════════════════════════════════════════════════════════════════
function normalizeMatn(s) {
  return (s || "")
    .replace(/[\u064B-\u065F\u0670]/g, "")  // strip harakat + superscript alif
    .replace(/\s+/g, " ").trim();
}
...
module.exports = async (req, res) => {
  ...
  if (wantSSE) {
    ...
    sseWrite(res, "status", { step: "TRADUCTION", msg: "Traduction Salafi (Ibn al-Athir)..." });
  }
  ...
  try {
    ...
    if (wantSSE) sseWrite(res, "status", { step: "DORAR", msg: "Dorar.net — jusqu\u2019\u00e0 5 r\u00e9sultats..." });
    ...
    if (wantSSE) {
      ...
      sseWrite(res, "status", {
        step:  "TAKHRIJ",
        msg:   "Jarh wa Ta\u2019dil \u2014 sources Salaf \u2014 " + results.length + " hadith(s)\u2026",
    ...
    if (wantSSE) {
      for (let i = 0; i < results.length; i++) {
        ...
        if (wantSSE) {
          sseWrite(res, "status", {
            step: "TAKHRIJ",
            msg:  "Analyse hadith " + (i+1) + "/" + results.length + " en cours\u2026",
            hadithIdx: i
          });
        }
        ...
        // Envoyer le hadith enrichi complet
