...
/* ── _resetZones : réinitialise l'état des cartes SSE + ronds dorés ── */
...
var _RE_SAHIH = /صحيح|حسن|جيّد|جيد|ثابت|إسناده صالح|رجاله ثقات|إسناده مقارب|إسناده حسن|إسناده صحيح|متفق عليه|الصحيحان|مخرج في الصحيح|في الصحيحين/;
...
function _mapHadithRaw(h) {
  ...
  var _LEVEL_TO_KEY = {sahih:'SAHIH', hasan:'HASAN', daif:'DAIF', mawdu:'MAWDU', mawquf:'MAWDU', rejected:'MAWDU', unknown:'INCONNU'};
  ...
  if(h.grade && h.grade !== 'INCONNU' && !card.dataset.mzBadge) {
    ...
    if(existingVerdict) {
      ...
      if(curKey !== h.grade) {
        ...
        var _freshV = _tmpV.firstChild;
        if(_freshV) {
          _freshV.style.animation = 'vIn .4s ease';
          existingVerdict.parentNode.replaceChild(_freshV, existingVerdict);
        }
      }
    }
  }
    ...
    setTimeout(function() {
      ...
      if(secAcc) {
        ...
        /* ── STRUCTURE 13 ZONES (Barème de Fer v24.1) ───────────────────
           ...
           Zone 13 : TAKHRÎJ      — sources et références croisées
        ...
        /* ── Zone TAKHRÎJ — Références croisées (backend field: takhrij) ── */
        z3Html += _mzAmanaAcc(
          '\ud83d\udcda TAKHR\u012aJ \u2014 Sources et R\u00e9f\u00e9rences',
...
function _renderDorarCards(rawHadiths, query) {
  ...
  /* Clés valides pré-calculées par _mapHadithRaw */
  var _MZ_GRADE_KEYS = ['SAHIH', 'HASAN', 'DAIF', 'MAWDU', 'INCONNU'];
  ...
  rawHadiths.forEach(function(r, idx) {
    ...
    var gradeRaw = r.grade_ar || (isMappedKey ? '' : r.grade) || '';

    /* Priorité 1 : r.grade déjà mappé par _mapHadithRaw → utilisation directe */
    var tg;
    if (isMappedKey && r.grade !== 'INCONNU') {
      tg = _getTechnicalGrade(gradeRaw);
      tg.key = r.grade;
    /* Priorité 2 : grade_level envoyé par le backend (dorar event) */
    } else if (r.grade_level && _MZ_LVL_MAP[(r.grade_level || '').toLowerCase()]) {
      tg = _getTechnicalGrade(gradeRaw);
      tg.key = _MZ_LVL_MAP[r.grade_level.toLowerCase()];
    /* Priorité 3 : classification locale depuis le texte arabe brut */
    } else {
      tg = _getTechnicalGrade(gradeRaw);
    }
    var metaStr = '';
