...
/* ═══════════════════════════════════════════════════════════════════
/* ════════════════════════════════════════════════════════════════════
   ZONE 2 v18.4 — ISNAD CÉLESTE
   Noms flottants (mzI-f) · Halo solaire (mzI-halo) · Fil d'or (mzI-th)
   Déduplication (_seen) · Tri chrono 5 couches · Offcanvas onclick
   #mz-isnad-container avec min-height:40px
   Zéro mzP-card-v11 · Zéro mz-chain-card · Zéro bordure rectangulaire
════════════════════════════════════════════════════════════════════ */

function _mzIsnadFallback(msg) {
  return `<div id="mz-isnad-container" style="min-height:40px;padding:14px 18px;text-align:center;">
    <p style="font-family:Cinzel,serif;font-size:6.5px;letter-spacing:.25em;color:rgba(201,168,76,.35);margin-bottom:8px;">ZONE 2 \u2014 SILSILAT AL-ISN\u0100D</p>
    <p style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:13px;color:rgba(201,168,76,.35);line-height:1.7;">${msg||'Donn\u00e9es de la cha\u00eene non disponibles.'}</p>
  </div>`;
}
...
function _mzIsnadFromPipe(isnadChain, grade) {
  ...
  /* GARDE 1 */
  if(isnadChain===null||isnadChain===undefined) return _mzIsnadFallback();
  ...
  /* CSS C\u00e9leste \u2014 injection unique */
  if(!document.getElementById('mzPipe-css')){
    var _sc=document.createElement('style');_sc.id='mzPipe-css';
    _sc.textContent=[
  ...
  /* GARDE 2 : parsing */
  var rawStr,lines2,nodes;
  try{
    rawStr=isnadChain.replace(/\\n/g,'\n');
    rawStr=rawStr.replace(/\s*[\u2014\u2013]\s*/g,' | ').replace(/\s+-\s+/g,' | ');
    lines2=rawStr.indexOf('\n')!==-1?rawStr.split('\n'):rawStr.split(/(?=Maillon\s+\d)/i);
    lines2=lines2.map(function(l){return typeof l==='string'?l.trim():'';}).filter(function(l){return l.length>2;});
  }catch(e){return _mzIsnadFallback();}

  /* GARDE 3 v18.4 : D\u00c9DUPLICATION + COUCHE */
  nodes=[];
  var _seen={};
  for(var _i=0;_i<lines2.length;_i++){
    try{
      var parts=lines2[_i].split('|');
      var nom=(parts[1]||'').trim();
      if(!nom||nom.length<2) continue;
      var key=nom.toLowerCase().replace(/[\u064B-\u065F\u0670]/g,'').replace(/[^a-z0-9\u0600-\u06FF]/g,'');
      if(_seen[key]){console.log('[v18.4] Doublon filtr\u00e9 : '+nom);continue;}
      _seen[key]=true;
      var titR=(parts[2]||'').trim(),verR=(parts[3]||'').trim().replace(/_/g,' '),sieR=(parts[4]||'').trim();
      var ep=parseSiecle(sieR);
      var couche=ep?ep.couche:3, numS=ep?ep.numRaw:5;
      var combo=(nom+' '+titR+' '+verR).toLowerCase();
      if(/albani|\u0623\u0644\u0628\u0627\u0646\u064a|ibn.*baz|\u0628\u0646.*\u0628\u0627\u0632|uthaymin|\u0639\u062b\u064a\u0645\u064a\u0646|muqbil|\u0645\u0642\u0628\u0644|rabi|\u0631\u0628\u064a\u0639|madkhali|fawzan|contemporain|verificateur/i.test(combo)){couche=5;numS=Math.max(numS,14);}
      nodes.push({num:String(nodes.length+1),nom:nom,titre:titR,verdict:verR,siecle:sieR,_c:couche,_n:numS});
    }catch(_){}
  }

  /* TRI CHRONOLOGIQUE STRICT */
  nodes.sort(function(a,b){return a._c!==b._c?a._c-b._c:a._n-b._n;});
  for(var _r=0;_r<nodes.length;_r++) nodes[_r].num=String(_r+1);

  /* GARDE 4 */
  if(!nodes.length){
    var safeT=isnadChain.substring(0,600).replace(/</g,'&lt;');
    return `<div id="mz-isnad-container" style="min-height:40px;padding:12px;"><p style="font-family:Cinzel,serif;font-size:6.5px;letter-spacing:.25em;color:rgba(201,168,76,.35);">ZONE 2 \u2014 SILSILAT AL-ISN\u0100D</p><p style="font-family:Cormorant Garamond,serif;font-size:13px;color:rgba(220,200,150,.7);line-height:1.7;white-space:pre-wrap;">${safeT}</p></div>`;
  }

  /* ═══════════════════════════════════════════════════════
     GARDE 5 v18.4 \u2014 HTML C\u00c9LESTE \u2014 BACKTICKS EXCLUSIFS
     Noms flottants \u00b7 Fil d\u2019or organique \u00b7 Halo solaire
     Z\u00e9ro bo\u00eete \u00b7 Z\u00e9ro carte \u00b7 Z\u00e9ro bordure rectangulaire
  ═══════════════════════════════════════════════════════ */
  try{
    var h=`<div id="mz-isnad-container" class="mzI-r" style="display:flex;flex-direction:column;align-items:center;width:100%;min-height:40px;padding:20px 12px 24px;position:relative;">`;

    h+=`<p style="font-family:Cinzel,serif;font-size:6px;letter-spacing:.4em;color:rgba(212,175,55,.35);margin-bottom:16px;">ZONE 2 \u2014 SILSILAT AL-ISN\u0100D \u2014 14 SI\u00c8CLES</p>`;

    /* PROPH\u00c8TE \uFDFA \u2014 HALO SOLAIRE */
    h+=`<div class="mzI-halo">\uFDFA</div>`;
    h+=`<p style="font-family:Scheherazade New,serif;font-size:22px;font-weight:700;color:#d4af37;text-shadow:0 0 30px rgba(212,175,55,.8),0 0 60px rgba(212,175,55,.3);margin-bottom:1px;animation:mzGlow 3s ease-in-out infinite;">\u0627\u0644\u0646\u0628\u064a \u0645\u062d\u0645\u062f \uFDFA</p>`;
    h+=`<p style="font-family:Cinzel,serif;font-size:8px;font-weight:700;color:rgba(212,175,55,.7);letter-spacing:.12em;">LE PROPH\u00c8TE MOHAMED \uFDFA</p>`;
    h+=`<p style="font-family:Cinzel,serif;font-size:5px;letter-spacing:.2em;color:rgba(212,175,55,.3);">SOURCE INFAILLIBLE \u2014 MA\u02bfS\u016bM</p>`;

    h+=`<div class="mzI-cn"><div class="mzI-th" style="height:22px;"></div><span class="mzI-cb">a transmis \u00e0</span><div class="mzI-th" style="height:10px;"></div></div>`;

    var lastC=0;
    var CL={1:'COUCHE 1 \u2014 AS-SAH\u0100BA',2:"COUCHE 2 \u2014 AT-T\u0100BI\u02bfIN & IMAMS",3:'COUCHE 3 \u2014 FONDATEURS',4:'COUCHE 4 \u2014 HUFFADH & PILIERS',5:'COUCHE 5 \u2014 CONTEMPORAINS (15e S. H\u00c9GIRE)'};
    var CC={1:'#d4af37',2:'#22c55e',3:'#60a5fa',4:'#f59e0b',5:'#a78bfa'};

    nodes.forEach(function(nd,i){
      var vis=nodeVis(nd.titre,nd.verdict,i,nodes.length);
      var ep2=parseSiecle(nd.siecle);
      var delay=(i*0.08).toFixed(2)+'s';

      if(nd._c!==lastC&&CL[nd._c]){
        var lc=CC[nd._c]||'#d4af37';
        h+=`<div style="width:100%;max-width:360px;display:flex;align-items:center;gap:8px;margin:14px auto 8px;"><div style="flex:1;height:1px;background:linear-gradient(to right,transparent,${lc}44);"></div><span style="font-family:Cinzel,serif;font-size:5.5px;font-weight:700;letter-spacing:.15em;white-space:nowrap;padding:3px 10px;border:1px solid ${lc}33;border-radius:5px;color:${lc};background:${lc}08;">${CL[nd._c]}</span><div style="flex:1;height:1px;background:linear-gradient(to left,transparent,${lc}44);"></div></div>`;
        lastC=nd._c;
      }

      var esc=function(s){return s.replace(/\\/g,'\\\\').replace(/'/g,"\\'").replace(/"/g,'&quot;');};

      var fontSize=nd._c===1?'20px':nd._c===5?'18px':'17px';
      h+=`<div class="mzI-f" style="animation-delay:${delay};" onclick="window.mzOpenIsnadPanel('${esc(nd.nom)}','${esc(nd.titre)}','${esc(nd.verdict)}','${esc(nd.siecle)}','${vis.dotC}')" role="button" tabindex="0">`;
      h+=`<p class="mzI-nm" style="font-size:${fontSize};color:${vis.nameC};">${nd.nom}</p>`;
      if(nd.verdict){h+=`<span class="mzI-tg" style="background:${vis.vBg};color:${vis.vC};border-color:${vis.vC}44;">${nd.verdict}</span>`;}
      if(ep2){h+=`<p class="mzI-er" style="color:${vis.cenC};">${ep2.num} si\u00e8cle de l\u2019H\u00e9gire \u2014 ${ep2.era}</p>`;}
      h+=`</div>`;

      if(i<nodes.length-1){
        var nextC=nodes[i+1]?nodes[i+1]._c:nd._c;
        var cTxt=nextC>nd._c?'ont transmis aux':'a transmis \u00e0';
        var thH=nextC!==nd._c?'18px':'12px';
        h+=`<div class="mzI-cn"><div class="mzI-th" style="height:${thH};opacity:${vis.beamOp};"></div><span class="mzI-cb" style="animation-delay:${delay};">${cTxt}</span><div class="mzI-th" style="height:6px;opacity:${vis.beamOp};"></div></div>`;
      }
    });

    h+=`<p style="font-family:Cinzel,serif;font-size:5.5px;letter-spacing:.22em;color:rgba(212,175,55,.18);text-align:center;margin-top:18px;padding-top:12px;border-top:1px solid rgba(212,175,55,.06);">SILSILAT AL-ISN\u0100D \u2014 ${nodes.length} MAILLONS \u2014 0 DOUBLON \u2014 14 SI\u00c8CLES</p>`;
    h+=`</div>`;
    return h;

  }catch(err){
    console.error('%c \ud83d\udd34 v18.4 _mzIsnadFromPipe CRASH','color:#ff0000;font-weight:bold;',err);
